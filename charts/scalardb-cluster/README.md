...
| scalardbCluster.encryption.vault.tls | object | `{"caRootCertSecret":"","enabled":false}` | TLS configurations to access from ScalarDB Cluster to Vault by using TLS. When you use HCP Vault Dedicated, you don't need to set these TLS configurations even though HCP Vault Dedicated uses HTTPS. You need to set these TLS configurations if and only if you use other HashiCorp Vault deployments than HCP Vault Dedicated, for example, self-hosted HashiCorp Vault. |
